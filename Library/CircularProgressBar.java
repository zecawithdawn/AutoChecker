...
public class CircularProgressBar extends ProgressBar{
    ...
    // 텍스트 사이즈 sp값 사용을 위해 추가.
    int titleSize = getResources().getDimensionPixelSize(R.dimen.circular_progress_prev_title_size);
    int subTitleSize = getResources().getDimensionPixelSize(R.dimen.circular_progress_prev_subtitle_size);
	...
	public CircularProgressBar(Context context, AttributeSet attrs) {
		super(context, attrs);
        if (getId() == R.id.circularprogressbar) {
            titleSize = getResources().getDimensionPixelSize(R.dimen.circular_progress_title_size);
            subTitleSize = getResources().getDimensionPixelSize(R.dimen.circular_progress_subtitle_size);
            color = "#FF7D3F";
        }
        init(attrs, 0);
	}
	...
	public void init(AttributeSet attrs, int style){
		...
		color = a.getString(R.styleable.CircularProgressBar_c_titleColor);
		if(color==null)
			mTitlePaint.setColor(res.getColor(R.color.circular_progress_default_title));
		else
			mTitlePaint.setColor(Color.parseColor(color));

		color = a.getString(R.styleable.CircularProgressBar_c_subtitleColor);
		if(color==null)
		...
		String t = a.getString(R.styleable.CircularProgressBar_c_title);
		if(t!=null)
			mTitle = t;

		t = a.getString(R.styleable.CircularProgressBar_c_subtitle);
		if(t!=null)
			mSubTitle = t;
...
//        mTitlePaint.setTypeface(Typeface.createFromAsset(getContext().getAssets(),"KoPubDotumMedium.ttf"));
		mTitlePaint.setTypeface(Typeface.create(Typeface.MONOSPACE, Typeface.NORMAL));
		mTitlePaint.setShadowLayer(0.1f, 0, 1, Color.GRAY);

		mSubtitlePaint.setTextSize(subTitleSize);
		mSubtitlePaint.setStyle(Style.FILL);
		mSubtitlePaint.setAntiAlias(true);
        //여기에 쳐 넣으세요
//        mSubtitlePaint.setTypeface(Typeface.createFromAsset(getContext().getAssets(),"KoPubDotumMedium.ttf"));
		mSubtitlePaint.setTypeface(Typeface.create(Typeface.MONOSPACE, Typeface.NORMAL));
